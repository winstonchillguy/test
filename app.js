...
const statusLines = [
  ...
  'Synchronizing proxy chain...',
  'Routing through stealth nodes...',
  'Injecting decoy packets...',
  'Compiling heuristic profiles...',
  'Parsing hex payload fragments...',
  'Rotating tokens...',
...
function engageModule(tool) {
  ...
  statusInterval = setInterval(() => {
    modalSubtitle.textContent = statusLines[Math.floor(Math.random() * statusLines.length)];
    if (Math.random() > 0.7) {
  ...
  attemptInterval = setInterval(() => {
    ...
    progressLabel.textContent = `Attempts: ${attemptCount.toLocaleString()}`;
    if (Math.random() > 0.85) {
...
function openModal(tool) {
  ...
  const statuses = [
    ...
    { label: 'Masking', value: `${Math.floor(Math.random() * 60) + 40}%` },
    { label: 'Cipher', value: 'AES-4096/ShadowTLS' },
    { label: 'Drift', value: `${(Math.random() * 0.4 + 0.1).toFixed(2)} ms` }
  ];
...
document.querySelectorAll('.nav button').forEach((button) => {
  button.addEventListener('click', () => {
    openModal({
      ...
      desc: 'Section interface simulation.',
      type: 'monitor'
    });
  });
});
